...
const handleSubmit = async () => {
  ...
  const estimatedTotalTime = 600000 // 与后端超时时间一致，单位为毫秒
  let lastProgress = 0

  // 模拟进度更新
  const progressInterval = setInterval(() => {
    const elapsed = Date.now() - startTime
    // 根据预估时间计算进度百分比，使用更平滑的函数
    const calculatedProgress = Math.min(90, Math.floor((elapsed / estimatedTotalTime) * 80))
    
    if (calculatedProgress > lastProgress) {
      loadingProgress.value = calculatedProgress
    ...
    if (loadingProgress.value <= 20) {
      ...
      // 进度达到80%以上时保持在90%，等待实际完成
      loadingProgress.value = 90
    }
  }, 2000) // 每2秒更新一次进度，避免过于频繁的UI更新
  ...
  try {
    ...
    // 更新进度为错误状态
    loadingProgress.value = 0
    loadingStatus.value = `❌ 错误: ${error.message || '生成旅行计划失败,请稍后重试'}`
